...
# ==========================================================
# PART 2: PREDICTOR VARIABLES & CORRELATION TESTING
# ==========================================================


# A. Prepare Predictor Data (May & June)
predictors_raw <- df_filtered %>%
  filter(month(sampledate) %in% c(5, 6), depth == 0) %>% # Surface variables only
  select(lakeid, year4, sampledate, wtemp) %>% # Using wtemp as the example variable
  mutate(month = month(sampledate))

# B. Calculate the 3 Temporal Options (May, June, May/June Avg)
pred_summary <- predictors_raw %>%
  group_by(lakeid, year4) %>%
  summarise(
    May_Val = mean(wtemp[month == 5], na.rm = TRUE),
    June_Val = mean(wtemp[month == 6], na.rm = TRUE),
    Avg_MayJun_Val = mean(wtemp, na.rm = TRUE), # Average of both months
    .groups = "drop"
  ) %>%
  # Fill missing years with Mean (per Emma's instructions)
  group_by(lakeid) %>%
  mutate(across(c(May_Val, June_Val, Avg_MayJun_Val), 
                ~ifelse(is.na(.), mean(., na.rm=TRUE), .))) %>%
  ungroup()

# C. Correlation Loop
# We will test which version (May, June, or Avg) correlates best with CVHT for each lake.

results_table <- data.frame() # Store best predictors here

for (lake in unique(pred_summary$lakeid)) {
  
  # Filter data for this lake
  lake_cvht <- cvht_final %>% filter(lakeid == lake)
  lake_preds <- pred_summary %>% filter(lakeid == lake)
  
  # Merge CVHT and Predictors
  merged_data <- inner_join(lake_cvht, lake_preds, by = "year4")
  
  # If not enough data, skip
  if(nrow(merged_data) < 5) next 
  
  # Calculate correlations
  cor_may <- cor(merged_data$CVHT_standardized, merged_data$May_Val, use="complete.obs")
  cor_jun <- cor(merged_data$CVHT_standardized, merged_data$June_Val, use="complete.obs")
  cor_avg <- cor(merged_data$CVHT_standardized, merged_data$Avg_MayJun_Val, use="complete.obs")
  
  # Find which absolute correlation is strongest
  cors <- c(May = abs(cor_may), June = abs(cor_jun), Avg = abs(cor_avg))
  best_period <- names(which.max(cors))
  best_cor_val <- max(cors) # Absolute value
  
  # Retrieve the actual (signed) correlation value
  actual_cor <- ifelse(best_period == "May", cor_may, 
                       ifelse(best_period == "June", cor_jun, cor_avg))
  
  # Save result
  results_table <- rbind(results_table, data.frame(
    Lake = lake,
    Variable = "Surface_Temp", # Change this if you loop through multiple variables
    Best_Time_Period = best_period,
    Correlation = actual_cor
  ))
}

print("--- Best Temporal Period Per Lake ---")
print(results_table)

# ==========================================================
# PART 3: SIGNIFICANCE TESTING & FINAL FILTERING
# ==========================================================

n <- 42 # Or nrow(merged_data) if you want it dynamic per lake
df <- n - 2
alpha <- 0.05

# Critical value calculation
critical_t <- qt(1 - alpha/2, df)
cor_critical_val <- critical_t / sqrt(critical_t^2 + df)

print(paste("Critical Correlation Threshold (+/-):", round(cor_critical_val, 3)))

# Filter the results table
significant_predictors <- results_table %>%
  filter(abs(Correlation) > cor_critical_val)

print("--- Significant Predictors to Keep for Model Runs ---")
